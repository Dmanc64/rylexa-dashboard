...
import {
  MessageSquare, Send, Loader2, Circle, ArrowLeft, Plus
} from 'lucide-react'
...
export default function VendorMessagesPage() {
  ...
  useEffect(() => {
    ...
    const channel = supabase
      .channel(`vendor-messages-${selectedId}`)
      .on('postgres_changes', {
        event: 'INSERT',
        schema: 'public',
        table: 'messages',
    ...
    <div className="min-h-screen bg-slate-50 font-sans text-slate-900">
      <div className="max-w-6xl mx-auto p-6 md:p-10">
        ...
        <div className="flex items-center gap-4 mb-8">
          ...
          <div>
            <p className="text-slate-400 font-bold text-[10px] tracking-[0.2em] uppercase">Vendor Portal</p>
            <h1 className="text-3xl font-black tracking-tight italic">Messages</h1>
          </div>
        </div>

        {/* TWO-COLUMN */}
        <div className="flex bg-white border border-slate-200 rounded-3xl overflow-hidden shadow-sm" style={{ height: 'calc(100vh - 220px)' }}>
          ...
          <div className="w-80 border-r border-slate-200 flex flex-col shrink-0">
            <div className="p-4 border-b border-slate-100 bg-slate-50 flex items-center justify-between gap-2">
              ...
              <button
                ...
                className="flex items-center gap-1.5 px-3 py-1.5 bg-orange-500 text-white rounded-lg text-[10px] font-black uppercase tracking-wider hover:bg-orange-600 transition-colors"
            ...
            <div className="flex-1 overflow-y-auto">
              {loadingConvos ? (
                ...
                <div className="p-10 text-center">
                  ...
                  <p className="text-slate-400 text-xs mt-1">Click <span className="font-black text-orange-500">New</span> to start a conversation.</p>
                </div>
              ) : (
                conversations.map(conv => (
                  <button
                    ...
                    className={`w-full text-left p-4 border-b border-slate-50 hover:bg-slate-50 transition-colors ${
                      selectedId === conv.id ? 'bg-orange-50/50 border-l-4 border-l-orange-500' : ''
                    }`}
                  >
                    <div className="flex items-start justify-between gap-2">
                      <div className="min-w-0 flex-1">
                        <div className="flex items-center gap-1.5 mb-1">
                          {conv.unread && <Circle size={7} className="fill-orange-500 text-orange-500 shrink-0" />}
            ...
            {!selectedId ? (
              ...
              <>
                ...
                <div className="p-4 bg-white border-t border-slate-100">
                  <div className="flex items-end gap-3 bg-slate-50 rounded-2xl px-4 py-2 focus-within:ring-2 focus-within:ring-orange-500/20 focus-within:bg-white transition-all border border-transparent focus-within:border-orange-500/50">
                    <textarea
                      value={messageInput}
                    ...
                    <button
                      ...
                      className="p-3 bg-slate-900 text-white rounded-xl hover:bg-orange-600 transition-all active:scale-95 shadow-md disabled:opacity-30 shrink-0 mb-1"
                    >
                      <Send size={18} />
                    </button>
                  </div>
